...
export class MarketingService {
  ...
  async findAllCampaigns(options?: {
    ...
    if (options?.isActive !== undefined) {
      query.andWhere('campaign.isActive = :isActive', {
  ...
  ): Promise<ServiceResponse<FlashSale>> {
  ...
  ): Promise<ServiceResponse<Voucher>> {
  ...
  async findAllVouchers(options?: {
    ...
    if (options?.isActive !== undefined) {
      query.andWhere('voucher.isActive = :isActive', {
        isActive: options.isActive,
      });
    }
  ...
  async validateVoucher(
    ...
    ServiceResponse<{ valid: boolean; discount: number; message: string }>
  > {
    const voucher = await this.voucherRepository.findOne({
    ...
    if (!voucher) {
      return {
        ...
        data: { valid: false, discount: 0, message: 'Voucher not found' },
      };
    }
    ...
    if (voucher.startsAt && now < voucher.startsAt) {
      return {
        ...
        data: { valid: false, discount: 0, message: 'Voucher not yet active' },
      };
    }
    if (voucher.expiresAt && now > voucher.expiresAt) {
      return {
        ...
        data: { valid: false, discount: 0, message: 'Voucher has expired' },
      };
    }

    if (voucher.totalLimit) {
      const usageCount = await this.voucherUsageRepository.count({
    ...
    return {
      ...
      data: { valid: true, discount, message: 'Voucher applied successfully' },
    };
  }
  ...
  async applyVoucher(
    ...
    userId: string,
    orderId: string,
  ): Promise<ServiceResponse<VoucherUsage>> {
