...
export class SeoService {
  ...
  async createMetadata(dto: CreateSeoMetadataDto): Promise<ServiceResponse<SeoMetadata>> {
  ...
  async findAllMetadata(options?: { entityType?: string; page?: number; limit?: number }): Promise<ServiceResponse<SeoMetadata[]>> {
    const query = this.seoMetadataRepository.createQueryBuilder('seo').orderBy('seo.createdAt', 'DESC');
    if (options?.entityType) query.andWhere('seo.entityType = :entityType', { entityType: options.entityType });
    const p = options?.page || 1;
    const l = options?.limit || 20;
    query.skip((p - 1) * l).take(l);
    const [metadata, total] = await query.getManyAndCount();
    return { success: true, message: 'SEO metadata retrieved', data: metadata, meta: { total, page: p, limit: l } };
  }

  async findMetadata(id: string): Promise<ServiceResponse<SeoMetadata>> {
    const metadata = await this.seoMetadataRepository.findOne({ where: { id } });
  ...
  async findMetadataByEntity(entityType: string, entityId: string): Promise<ServiceResponse<SeoMetadata>> {
    const metadata = await this.seoMetadataRepository.findOne({ where: { entityType, entityId } });
    if (!metadata) throw new NotFoundException('SEO metadata not found');
    return { success: true, message: 'SEO metadata retrieved', data: metadata };
  }

  async findMetadataByUrl(canonicalUrl: string): Promise<ServiceResponse<SeoMetadata>> {
    const metadata = await this.seoMetadataRepository.findOne({ where: { canonicalUrl } });
    if (!metadata) throw new NotFoundException('SEO metadata not found');
    return { success: true, message: 'SEO metadata retrieved', data: metadata };
  }

  async updateMetadata(id: string, dto: UpdateSeoMetadataDto): Promise<ServiceResponse<SeoMetadata>> {
    const metadata = await this.seoMetadataRepository.findOne({ where: { id } });
    if (!metadata) throw new NotFoundException('SEO metadata not found');
    Object.assign(metadata, dto);
  ...
  async upsertMetadata(entityType: string, entityId: string, dto: UpdateSeoMetadataDto): Promise<ServiceResponse<SeoMetadata>> {
    let metadata = await this.seoMetadataRepository.findOne({ where: { entityType, entityId } });
    if (metadata) {
      Object.assign(metadata, dto);
    } else {
  ...
  async createRedirect(dto: CreateUrlRedirectDto): Promise<ServiceResponse<UrlRedirect>> {
  ...
  async findAllRedirects(options?: { isActive?: boolean; page?: number; limit?: number }): Promise<ServiceResponse<UrlRedirect[]>> {
    const query = this.urlRedirectRepository.createQueryBuilder('redirect').orderBy('redirect.createdAt', 'DESC');
    if (options?.isActive !== undefined) query.andWhere('redirect.isActive = :isActive', { isActive: options.isActive });
    const p = options?.page || 1;
    const l = options?.limit || 50;
    query.skip((p - 1) * l).take(l);
    const [redirects, total] = await query.getManyAndCount();
    return { success: true, message: 'URL redirects retrieved', data: redirects, meta: { total, page: p, limit: l } };
  }

  async findRedirect(id: string): Promise<ServiceResponse<UrlRedirect>> {
    const redirect = await this.urlRedirectRepository.findOne({ where: { id } });
  ...
  async findRedirectBySource(sourceUrl: string): Promise<ServiceResponse<UrlRedirect>> {
    const redirect = await this.urlRedirectRepository.findOne({ where: { sourceUrl, isActive: true } });
    if (!redirect) throw new NotFoundException('URL redirect not found');
  ...
  async updateRedirect(id: string, dto: UpdateUrlRedirectDto): Promise<ServiceResponse<UrlRedirect>> {
    const redirect = await this.urlRedirectRepository.findOne({ where: { id } });
    if (!redirect) throw new NotFoundException('URL redirect not found');
    Object.assign(redirect, dto);
  ...
  async toggleRedirectActive(id: string): Promise<ServiceResponse<UrlRedirect>> {
    const redirect = await this.urlRedirectRepository.findOne({ where: { id } });
    if (!redirect) throw new NotFoundException('URL redirect not found');
    redirect.isActive = !redirect.isActive;
    const updated = await this.urlRedirectRepository.save(redirect);
    return { success: true, message: `Redirect ${redirect.isActive ? 'activated' : 'deactivated'}`, data: updated };
  }

  async bulkCreateRedirects(redirects: CreateUrlRedirectDto[]): Promise<ServiceResponse<UrlRedirect[]>> {
    ...
    return { success: true, message: `${saved.length} redirects created`, data: saved };
  }
}
