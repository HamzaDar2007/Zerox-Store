...
export class SubscriptionsController extends BaseController {
  constructor(private readonly subscriptionsService: SubscriptionsService) { super(); }

  @Post()
  @ApiOperation({ summary: 'Create subscription' })
  create(@Body() dto: CreateSubscriptionDto, @CurrentUser() user: User) {
    return this.handleAsyncOperation(this.subscriptionsService.create(user.id, dto));
  }
  ...
  findAll(@Query('status') status?: string, @Query('page') page?: number, @Query('limit') limit?: number) {
    return this.handleAsyncOperation(this.subscriptionsService.findAll({ status, page, limit }));
  }

  @Get('my-subscriptions')
  @ApiOperation({ summary: 'Get my subscriptions' })
  getMySubscriptions(@CurrentUser() user: User) {
    return this.handleAsyncOperation(this.subscriptionsService.findByUser(user.id));
  }
  ...
  @Permissions('subscriptions.read')
  getDueSubscriptions() {
    return this.handleAsyncOperation(this.subscriptionsService.getDueSubscriptions());
  }
  ...
  processRenewal(@Param('id', ParseUUIDPipe) id: string) {
    return this.handleAsyncOperation(this.subscriptionsService.processRenewal(id));
  }
