...
export class SearchService {
  ...
  async saveSearchHistory(userId: string, query: string): Promise<ServiceResponse<SearchHistory>> {
  ...
  async getSearchHistory(userId: string, limit: number = 10): Promise<ServiceResponse<SearchHistory[]>> {
    ...
    return { success: true, message: 'Search history retrieved', data: history };
  }
  ...
  async addRecentlyViewed(userId: string, productId: string): Promise<ServiceResponse<RecentlyViewed>> {
    let item = await this.recentlyViewedRepository.findOne({ where: { userId, productId } });
    if (item) {
      item.lastViewedAt = new Date();
      item.viewCount = (item.viewCount || 1) + 1;
    } else {
      item = new RecentlyViewed();
      Object.assign(item, { userId, productId, lastViewedAt: new Date(), viewCount: 1 });
    }
    const saved = await this.recentlyViewedRepository.save(item);
    return { success: true, message: 'Recently viewed updated', data: saved };
  }

  async getRecentlyViewed(userId: string, limit: number = 20): Promise<ServiceResponse<RecentlyViewed[]>> {
  ...
  async addToComparison(userId: string, productId: string): Promise<ServiceResponse<ProductComparison>> {
    // Find existing comparison or create new one
    let comparison = await this.comparisonRepository.findOne({ where: { userId } });
    if (comparison) {
      ...
      if (!comparison.productIds.includes(productId)) {
        if (comparison.productIds.length >= 4) {
          ...
          comparison.productIds = [...comparison.productIds.slice(1), productId];
        } else {
  ...
  async getComparison(userId: string): Promise<ServiceResponse<ProductComparison[]>> {
  ...
  async removeFromComparison(userId: string, productId: string): Promise<ServiceResponse<void>> {
    const comparison = await this.comparisonRepository.findOne({ where: { userId } });
    if (comparison) {
  ...
  async getRecommendations(productId: string): Promise<ServiceResponse<ProductRecommendation[]>> {
    ...
    return { success: true, message: 'Recommendations retrieved', data: recommendations };
  }
}
