...
export class BundlesService {
  ...
  async create(dto: CreateProductBundleDto): Promise<ServiceResponse<ProductBundle>> {
  ...
  async findAll(options?: { isActive?: boolean; page?: number; limit?: number }): Promise<ServiceResponse<ProductBundle[]>> {
    ...
    if (options?.isActive !== undefined) query.andWhere('bundle.isActive = :isActive', { isActive: options.isActive });
    const page = options?.page || 1;
    const limit = options?.limit || 20;
    query.skip((page - 1) * limit).take(limit);
    const [bundles, total] = await query.getManyAndCount();
    return { success: true, message: 'Bundles retrieved', data: bundles, meta: { total, page, limit } };
  }

  async findOne(id: string): Promise<ServiceResponse<ProductBundle>> {
    const bundle = await this.bundleRepository.findOne({ where: { id }, relations: ['items', 'items.product', 'items.variant'] });
  ...
  async findBySlug(slug: string): Promise<ServiceResponse<ProductBundle>> {
    const bundle = await this.bundleRepository.findOne({ where: { slug }, relations: ['items', 'items.product', 'items.variant'] });
    if (!bundle) throw new NotFoundException('Bundle not found');
    return { success: true, message: 'Bundle retrieved', data: bundle };
  }

  async update(id: string, dto: UpdateProductBundleDto): Promise<ServiceResponse<ProductBundle>> {
  ...
  async addItem(bundleId: string, dto: { productId: string; variantId?: string; quantity?: number }): Promise<ServiceResponse<BundleItem>> {
  ...
  async updateItem(bundleId: string, itemId: string, dto: Partial<{ productId: string; variantId: string; quantity: number; sortOrder: number }>): Promise<ServiceResponse<BundleItem>> {
    const item = await this.itemRepository.findOne({ where: { id: itemId, bundleId } });
  ...
  async removeItem(bundleId: string, itemId: string): Promise<ServiceResponse<void>> {
    ...
    await this.recalculateBundlePrice(bundleId);
    return { success: true, message: 'Item removed from bundle', data: undefined };
  }

  async getItems(bundleId: string): Promise<ServiceResponse<BundleItem[]>> {
    const items = await this.itemRepository.find({ where: { bundleId }, relations: ['product', 'variant'] });
    return { success: true, message: 'Bundle items retrieved', data: items };
  }

  async calculateBundlePrice(bundleId: string): Promise<ServiceResponse<{ originalPrice: number; bundlePrice: number; savings: number }>> {
    const bundle = await this.bundleRepository.findOne({ where: { id: bundleId }, relations: ['items', 'items.product'] });
    if (!bundle) throw new NotFoundException('Bundle not found');
    const originalPrice = bundle.items.reduce((sum, item) => sum + (Number(item.product?.price || 0) * item.quantity), 0);
    const bundlePrice = Number(bundle.bundlePrice) || originalPrice - (originalPrice * Number(bundle.discountValue || 0) / 100);
    return { success: true, message: 'Price calculated', data: { originalPrice, bundlePrice, savings: originalPrice - bundlePrice } };
  }
  ...
  private async recalculateBundlePrice(bundleId: string): Promise<void> {
    ...
    if (bundle.discountValue && !bundle.bundlePrice) {
      bundle.bundlePrice = originalPrice - (originalPrice * Number(bundle.discountValue) / 100);
    }
  ...
  async toggleActive(id: string): Promise<ServiceResponse<ProductBundle>> {
    ...
    const updated = await this.bundleRepository.save(bundle);
    return { success: true, message: `Bundle ${bundle.isActive ? 'activated' : 'deactivated'}`, data: updated };
  }
}
