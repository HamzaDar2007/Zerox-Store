...
export class SystemService {
  ...
  async createSetting(dto: CreateSystemSettingDto): Promise<ServiceResponse<SystemSetting>> {
  ...
  async findAllSettings(options?: { group?: string; isPublic?: boolean }): Promise<ServiceResponse<SystemSetting[]>> {
    const where: any = {};
    if (options?.group) where.group = options.group;
    if (options?.isPublic !== undefined) where.isPublic = options.isPublic;
    const settings = await this.settingRepository.find({ where, order: { group: 'ASC', key: 'ASC' } });
  ...
  async updateSetting(id: string, dto: UpdateSystemSettingDto): Promise<ServiceResponse<SystemSetting>> {
    const setting = await this.settingRepository.findOne({ where: { id } });
    if (!setting) throw new NotFoundException('Setting not found');
    Object.assign(setting, dto);
  ...
  async updateSettingByKey(key: string, value: string): Promise<ServiceResponse<SystemSetting>> {
    const setting = await this.settingRepository.findOne({ where: { key } });
    if (!setting) throw new NotFoundException('Setting not found');
    setting.value = value;
    const updated = await this.settingRepository.save(setting);
  ...
  async getSettingsByGroup(group: string): Promise<ServiceResponse<Record<string, string>>> {
  ...
  async bulkUpdateSettings(settings: { key: string; value: string }[]): Promise<ServiceResponse<SystemSetting[]>> {
    ...
    return { success: true, message: `${updated.length} settings updated`, data: updated };
  }

  async createFeatureFlag(dto: CreateFeatureFlagDto): Promise<ServiceResponse<FeatureFlag>> {
  ...
  async findAllFeatureFlags(): Promise<ServiceResponse<FeatureFlag[]>> {
    const flags = await this.featureFlagRepository.find({ order: { name: 'ASC' } });
  ...
  async updateFeatureFlag(id: string, dto: UpdateFeatureFlagDto): Promise<ServiceResponse<FeatureFlag>> {
    const flag = await this.featureFlagRepository.findOne({ where: { id } });
    if (!flag) throw new NotFoundException('Feature flag not found');
    Object.assign(flag, dto);
  ...
  async toggleFeatureFlag(id: string): Promise<ServiceResponse<FeatureFlag>> {
    ...
    const updated = await this.featureFlagRepository.save(flag);
    return { success: true, message: `Feature ${flag.isEnabled ? 'enabled' : 'disabled'}`, data: updated };
  }
  ...
  async getEnabledFeatures(): Promise<ServiceResponse<string[]>> {
    const flags = await this.featureFlagRepository.find({ where: { isEnabled: true } });
    return { success: true, message: 'Enabled features retrieved', data: flags.map(f => f.name) };
  }
}
