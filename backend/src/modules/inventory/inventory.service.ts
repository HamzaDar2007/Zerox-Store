...
import { TransferStatus, ReservationStatus, StockMovementType } from '@common/enums';
...
export class InventoryService {
  ...
  async createWarehouse(dto: CreateWarehouseDto): Promise<ServiceResponse<Warehouse>> {
  ...
  async findAllWarehouses(sellerId?: string): Promise<ServiceResponse<Warehouse[]>> {
  ...
  async findOneWarehouse(id: string): Promise<ServiceResponse<Warehouse>> {
    const warehouse = await this.warehouseRepository.findOne({ where: { id }, relations: ['inventory'] });
    if (!warehouse) throw new NotFoundException('Warehouse not found');
    return { success: true, message: 'Warehouse retrieved', data: warehouse };
  }

  async updateWarehouse(id: string, dto: UpdateWarehouseDto): Promise<ServiceResponse<Warehouse>> {
  ...
  async deleteWarehouse(id: string): Promise<ServiceResponse<void>> {
    const warehouse = await this.warehouseRepository.findOne({ where: { id } });
    if (!warehouse) throw new NotFoundException('Warehouse not found');
    const hasInventory = await this.inventoryRepository.count({ where: { warehouseId: id } });
    if (hasInventory > 0) throw new BadRequestException('Cannot delete warehouse with inventory');
  ...
  async getWarehouseInventory(warehouseId: string): Promise<ServiceResponse<Inventory[]>> {
    const inventory = await this.inventoryRepository.find({
      where: { warehouseId },
      relations: ['product', 'productVariant'],
    });
    return { success: true, message: 'Warehouse inventory retrieved', data: inventory };
  }

  // ==================== INVENTORY ====================

  async getInventory(productId: string, warehouseId?: string): Promise<ServiceResponse<Inventory[]>> {
    ...
    if (warehouseId) query.andWhere('inv.warehouseId = :warehouseId', { warehouseId });
    const inventory = await query.getMany();
    return { success: true, message: 'Inventory retrieved', data: inventory };
  }

  async getProductInventory(productId: string, variantId?: string): Promise<ServiceResponse<Inventory[]>> {
    const query = this.inventoryRepository.createQueryBuilder('inv')
      .leftJoinAndSelect('inv.warehouse', 'warehouse')
      .where('inv.productId = :productId', { productId });
    if (variantId) query.andWhere('inv.productVariantId = :variantId', { variantId });
    const inventory = await query.getMany();
    return { success: true, message: 'Product inventory retrieved', data: inventory };
  }

  async setStock(productId: string, warehouseId: string, quantity: number, productVariantId?: string): Promise<ServiceResponse<Inventory>> {
    let inventory = await this.inventoryRepository.findOne({
      where: { productId, warehouseId, productVariantId: productVariantId || null },
    });
    if (!inventory) {
  ...
  async adjustStock(
    ...
    try {
      ...
      movement.type = adjustment > 0 ? StockMovementType.ADJUSTMENT_ADD : StockMovementType.ADJUSTMENT_SUBTRACT;
  ...
  async getStockMovements(inventoryId: string, page = 1, limit = 50): Promise<ServiceResponse<StockMovement[]>> {
    ...
    return { success: true, message: 'Stock movements retrieved', data: movements };
  }

  async getMovementHistory(productId: string, warehouseId?: string, page = 1, limit = 50): Promise<ServiceResponse<StockMovement[]>> {
    ...
    if (warehouseId) query.andWhere('inventory.warehouseId = :warehouseId', { warehouseId });
    const movements = await query
      .orderBy('movement.createdAt', 'DESC')
      .skip((page - 1) * limit)
      .take(limit)
      .getMany();
    return { success: true, message: 'Movement history retrieved', data: movements };
  }

  async createMovement(dto: CreateStockMovementDto, createdBy: string): Promise<ServiceResponse<StockMovement>> {
    const movement = new StockMovement();
  ...
  async getTransfers(warehouseId?: string): Promise<ServiceResponse<InventoryTransfer[]>> {
    ...
    if (warehouseId) {
      query.where('transfer.fromWarehouseId = :warehouseId OR transfer.toWarehouseId = :warehouseId', { warehouseId });
    }
    const transfers = await query.getMany();
    return { success: true, message: 'Transfers retrieved', data: transfers };
  }

  async getLowStockItems(warehouseId?: string): Promise<ServiceResponse<Inventory[]>> {
    const query = this.inventoryRepository.createQueryBuilder('inv')
    ...
    if (warehouseId) query.andWhere('inv.warehouseId = :warehouseId', { warehouseId });
    const inventory = await query.getMany();
    return { success: true, message: 'Low stock items retrieved', data: inventory };
  }

  // ==================== RESERVATIONS ====================

  async reserveStock(productId: string, warehouseId: string, quantity: number, orderId: string, productVariantId?: string): Promise<ServiceResponse<StockReservation>> {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
  ...
  async releaseReservation(reservationId: string): Promise<ServiceResponse<void>> {
    ...
    const inventory = await this.inventoryRepository.findOne({ where: { id: reservation.inventoryId } });
    if (inventory) {
  ...
  async commitReservation(reservationId: string): Promise<ServiceResponse<void>> {
    const reservation = await this.reservationRepository.findOne({ where: { id: reservationId } });
    if (!reservation) throw new NotFoundException('Reservation not found');
    if (reservation.status !== ReservationStatus.HELD) {
  ...
  async createTransfer(dto: CreateInventoryTransferDto, initiatedBy: string): Promise<ServiceResponse<InventoryTransfer>> {
  ...
  async findAllTransfers(options?: { fromWarehouseId?: string; toWarehouseId?: string; status?: TransferStatus; page?: number; limit?: number }): Promise<ServiceResponse<InventoryTransfer[]>> {
    const query = this.transferRepository.createQueryBuilder('transfer')
      .leftJoinAndSelect('transfer.fromWarehouse', 'fromWarehouse')
      .leftJoinAndSelect('transfer.toWarehouse', 'toWarehouse')
      .leftJoinAndSelect('transfer.items', 'items')
      .orderBy('transfer.createdAt', 'DESC');
    if (options?.fromWarehouseId) query.andWhere('transfer.fromWarehouseId = :fromWarehouseId', { fromWarehouseId: options.fromWarehouseId });
    if (options?.toWarehouseId) query.andWhere('transfer.toWarehouseId = :toWarehouseId', { toWarehouseId: options.toWarehouseId });
    if (options?.status) query.andWhere('transfer.status = :status', { status: options.status });
    const page = options?.page || 1;
    const limit = options?.limit || 20;
    query.skip((page - 1) * limit).take(limit);
    const [transfers, total] = await query.getManyAndCount();
    return { success: true, message: 'Transfers retrieved', data: transfers, meta: { total, page, limit } };
  }

  async findOneTransfer(id: string): Promise<ServiceResponse<InventoryTransfer>> {
    const transfer = await this.transferRepository.findOne({
      where: { id },
      relations: ['fromWarehouse', 'toWarehouse', 'items', 'items.product', 'items.productVariant', 'initiator', 'approver'],
    });
    if (!transfer) throw new NotFoundException('Transfer not found');
    return { success: true, message: 'Transfer retrieved', data: transfer };
  }

  async approveTransfer(id: string, approvedBy: string): Promise<ServiceResponse<InventoryTransfer>> {
  ...
  async completeTransfer(id: string): Promise<ServiceResponse<InventoryTransfer>> {
    ...
    try {
      for (const item of transfer.items) {
        ...
        const sourceInv = await queryRunner.manager.findOne(Inventory, {
          where: { productId: item.productId, warehouseId: transfer.fromWarehouseId, productVariantId: item.productVariantId },
        });
        if (sourceInv) {
        ...
        let destInv = await queryRunner.manager.findOne(Inventory, {
          where: { productId: item.productId, warehouseId: transfer.toWarehouseId, productVariantId: item.productVariantId },
        });
        if (!destInv) {
