...
export class CmsService {
  ...
  async findAllPages(options?: { isPublished?: boolean; page?: number; limit?: number }): Promise<ServiceResponse<Page[]>> {
    const query = this.pageRepository.createQueryBuilder('page').orderBy('page.createdAt', 'DESC');
    if (options?.isPublished !== undefined) query.andWhere('page.isPublished = :isPublished', { isPublished: options.isPublished });
    const p = options?.page || 1;
    const l = options?.limit || 20;
    query.skip((p - 1) * l).take(l);
    const [pages, total] = await query.getManyAndCount();
    return { success: true, message: 'Pages retrieved', data: pages, meta: { total, page: p, limit: l } };
  }
  ...
  async findPageBySlug(slug: string): Promise<ServiceResponse<Page>> {
    const page = await this.pageRepository.findOne({ where: { slug, isPublished: true } });
    if (!page) throw new NotFoundException('Page not found');
    return { success: true, message: 'Page retrieved', data: page };
  }

  async updatePage(id: string, dto: UpdatePageDto): Promise<ServiceResponse<Page>> {
    const page = await this.pageRepository.findOne({ where: { id } });
    if (!page) throw new NotFoundException('Page not found');
    Object.assign(page, dto);
  ...
  async findAllBanners(options?: { isActive?: boolean; position?: string; page?: number; limit?: number }): Promise<ServiceResponse<Banner[]>> {
    const query = this.bannerRepository.createQueryBuilder('banner').orderBy('banner.sortOrder', 'ASC');
    if (options?.isActive !== undefined) query.andWhere('banner.isActive = :isActive', { isActive: options.isActive });
    if (options?.position) query.andWhere('banner.position = :position', { position: options.position });
    const p = options?.page || 1;
    const l = options?.limit || 50;
    query.skip((p - 1) * l).take(l);
    const [banners, total] = await query.getManyAndCount();
    return { success: true, message: 'Banners retrieved', data: banners, meta: { total, page: p, limit: l } };
  }
  ...
  async updateBanner(id: string, dto: UpdateBannerDto): Promise<ServiceResponse<Banner>> {
    const banner = await this.bannerRepository.findOne({ where: { id } });
    if (!banner) throw new NotFoundException('Banner not found');
    Object.assign(banner, dto);
  ...
  async toggleBannerActive(id: string): Promise<ServiceResponse<Banner>> {
    ...
    const updated = await this.bannerRepository.save(banner);
    return { success: true, message: `Banner ${banner.isActive ? 'activated' : 'deactivated'}`, data: updated };
  }

  async getActiveBannersByPosition(position: string): Promise<ServiceResponse<Banner[]>> {
    const now = new Date();
    const banners = await this.bannerRepository.createQueryBuilder('banner')
      ...
      .getMany();
    return { success: true, message: 'Active banners retrieved', data: banners };
  }
}
