...
export class LoyaltyService {
  ...
  async getOrCreatePoints(userId: string): Promise<ServiceResponse<LoyaltyPoints>> {
    let points = await this.pointsRepository.findOne({ where: { userId }, relations: ['tier'] });
    if (!points) {
      const defaultTier = await this.tierRepository.findOne({ where: { minPoints: 0 }, order: { minPoints: 'ASC' } });
  ...
  async earnPoints(userId: string, dto: CreateLoyaltyTransactionDto): Promise<ServiceResponse<LoyaltyPoints>> {
    ...
    try {
      ...
      return { success: true, message: `${dto.points} points earned`, data: points };
    } catch (error) {
  ...
  async redeemPoints(userId: string, dto: { points: number; orderId?: string; description?: string }): Promise<ServiceResponse<LoyaltyPoints>> {
    const pointsRecord = await this.getOrCreatePoints(userId);
    const points = pointsRecord.data;
    if (points.availableBalance < dto.points) throw new BadRequestException('Insufficient points');
    points.availableBalance -= dto.points;
    ...
    return { success: true, message: `${dto.points} points redeemed`, data: points };
  }

  async getTransactions(userId: string, page = 1, limit = 20): Promise<ServiceResponse<LoyaltyTransaction[]>> {
    ...
    return { success: true, message: 'Transactions retrieved', data: transactions, meta: { total, page, limit } };
  }
  ...
  async getTiers(): Promise<ServiceResponse<LoyaltyTier[]>> {
    const tiers = await this.tierRepository.find({ order: { minPoints: 'ASC' } });
    return { success: true, message: 'Tiers retrieved', data: tiers };
  }

  async createTier(dto: CreateLoyaltyTierDto): Promise<ServiceResponse<LoyaltyTier>> {
  ...
  async updateTier(id: string, dto: UpdateLoyaltyTierDto): Promise<ServiceResponse<LoyaltyTier>> {
  ...
  async getOrCreateReferralCode(userId: string): Promise<ServiceResponse<ReferralCode>> {
    let code = await this.referralCodeRepository.findOne({ where: { userId, isActive: true } });
    if (!code) {
  ...
  async applyReferralCode(referredUserId: string, codeString: string): Promise<ServiceResponse<Referral>> {
    const code = await this.referralCodeRepository.findOne({ where: { code: codeString, isActive: true } });
    if (!code) throw new NotFoundException('Invalid referral code');
    if (code.userId === referredUserId) throw new BadRequestException('Cannot use your own referral code');
    const existing = await this.referralRepository.findOne({ where: { referredUserId } });
  ...
  async completeReferral(referredUserId: string, orderId: string): Promise<ServiceResponse<Referral>> {
    const referral = await this.referralRepository.findOne({ where: { referredUserId, status: ReferralStatus.PENDING }, relations: ['referralCode'] });
    ...
    await this.earnPoints(referral.referrerUserId, { points: 500, referenceType: 'referral', description: 'Referral bonus' } as CreateLoyaltyTransactionDto);

    // Update referral code stats
    const code = await this.referralCodeRepository.findOne({ where: { id: referral.referralCodeId } });
    if (code) {
  ...
  async getReferrals(userId: string): Promise<ServiceResponse<Referral[]>> {
    const referrals = await this.referralRepository.find({ where: { referrerUserId: userId }, relations: ['referredUser'] });
