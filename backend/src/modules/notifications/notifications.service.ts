...
export class NotificationsService {
  ...
  async create(userId: string, dto: any): Promise<ServiceResponse<Notification>> {
  ...
  async findAll(userId: string, options?: { isRead?: boolean; type?: string; page?: number; limit?: number }): Promise<ServiceResponse<Notification[]>> {
    ...
    if (options?.type) query.andWhere('notification.type = :type', { type: options.type });
  ...
  async markAllAsRead(userId: string): Promise<ServiceResponse<void>> {
    await this.notificationRepository.update({ userId, readAt: IsNull() }, { readAt: new Date() });
  ...
  async getPreferences(userId: string): Promise<ServiceResponse<NotificationPreference[]>> {
  ...
  async updatePreference(userId: string, type: string, dto: any): Promise<ServiceResponse<NotificationPreference>> {
    let pref = await this.preferenceRepository.findOne({ where: { userId, type: type as any } });
    if (!pref) {
  ...
  async createTemplate(dto: CreateNotificationTemplateDto): Promise<ServiceResponse<NotificationTemplate>> {
  ...
  async updateTemplate(id: string, dto: UpdateNotificationTemplateDto): Promise<ServiceResponse<NotificationTemplate>> {
