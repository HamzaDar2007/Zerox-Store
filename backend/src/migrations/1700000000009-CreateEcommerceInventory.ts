...
export class CreateEcommerceInventory1700000000009 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'address_line1', type: 'varchar', length: '200', isNullable: false },
          { name: 'address_line2', type: 'varchar', length: '200', isNullable: true },
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'cost_per_unit', type: 'decimal', precision: 12, scale: 2, isNullable: true },
          { name: 'note', type: 'text', isNullable: true },
          { name: 'performed_by', type: 'uuid', isNullable: true },
          { name: 'created_at', type: 'timestamptz', isNullable: false, default: 'NOW()' },
        ],
        foreignKeys: [
          new TableForeignKey({ columnNames: ['inventory_id'], referencedTableName: 'inventory', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'quantity', type: 'integer', isNullable: false },
          { name: 'status', type: 'reservation_status_enum', isNullable: false, default: `'held'` },
        ...
        foreignKeys: [
          ...
          new TableForeignKey({ columnNames: ['product_variant_id'], referencedTableName: 'product_variants', referencedColumnNames: ['id'] }),
        ],
      }),
      true,
    );
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          { name: 'id', type: 'uuid', isPrimary: true, default: 'uuid_generate_v4()' },
          { name: 'transfer_number', type: 'varchar', length: '50', isNullable: false, isUnique: true },
          { name: 'from_warehouse_id', type: 'uuid', isNullable: false },
          { name: 'to_warehouse_id', type: 'uuid', isNullable: false },
          { name: 'status', type: 'transfer_status_enum', default: `'pending'` },
        ...
        foreignKeys: [
          new TableForeignKey({ columnNames: ['from_warehouse_id'], referencedTableName: 'warehouses', referencedColumnNames: ['id'] }),
          new TableForeignKey({ columnNames: ['to_warehouse_id'], referencedTableName: 'warehouses', referencedColumnNames: ['id'] }),
          new TableForeignKey({ columnNames: ['initiated_by'], referencedTableName: 'users', referencedColumnNames: ['id'] }),
    ...
    await queryRunner.createTable(
      new Table({
        ...
        uniques: [new TableUnique({ columnNames: ['transfer_id', 'product_id', 'product_variant_id'] })],
        foreignKeys: [
          new TableForeignKey({ columnNames: ['transfer_id'], referencedTableName: 'inventory_transfers', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
          new TableForeignKey({ columnNames: ['product_id'], referencedTableName: 'products', referencedColumnNames: ['id'] }),
          new TableForeignKey({ columnNames: ['product_variant_id'], referencedTableName: 'product_variants', referencedColumnNames: ['id'] }),
        ],
      }),
    ...
    // Indexes
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_warehouses_seller ON warehouses(seller_id) WHERE seller_id IS NOT NULL`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_warehouses_active ON warehouses(id) WHERE is_active = TRUE`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_warehouses_city ON warehouses(city)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_inventory_product ON inventory(product_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_inventory_variant ON inventory(product_variant_id) WHERE product_variant_id IS NOT NULL`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_inventory_warehouse ON inventory(warehouse_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_inventory_low_stock ON inventory(warehouse_id) WHERE quantity_on_hand <= low_stock_threshold`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_inventory_available ON inventory(product_variant_id, warehouse_id, quantity_available)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_stock_movements_inventory ON stock_movements(inventory_id, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_stock_movements_type ON stock_movements(type, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_stock_movements_reference ON stock_movements(reference_type, reference_id)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reservations_active ON stock_reservations(expires_at) WHERE status = 'held'`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reservations_inventory ON stock_reservations(inventory_id, status)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reservations_order ON stock_reservations(order_id) WHERE order_id IS NOT NULL`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reservations_cart ON stock_reservations(cart_id) WHERE cart_id IS NOT NULL`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_transfers_status ON inventory_transfers(status, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_transfers_from ON inventory_transfers(from_warehouse_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_transfers_to ON inventory_transfers(to_warehouse_id)`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP TRIGGER IF EXISTS trg_transfers_updated_at ON inventory_transfers`);
    await queryRunner.query(`DROP TRIGGER IF EXISTS trg_inventory_updated_at ON inventory`);
