...
export class CreateEcommercePayments1700000000012 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'transaction_id', type: 'varchar', length: '200', isNullable: true, isUnique: true },
          { name: 'gateway_name', type: 'varchar', length: '50', isNullable: true },
          ...
          { name: 'gateway_fee', type: 'decimal', precision: 12, scale: 2, default: 0.00 },
          { name: 'platform_fee', type: 'decimal', precision: 12, scale: 2, default: 0.00 },
          { name: 'net_amount', type: 'decimal', precision: 14, scale: 2, isNullable: true },
    ...
    await queryRunner.createTable(
      new Table({
        ...
        foreignKeys: [
          new TableForeignKey({ columnNames: ['payment_id'], referencedTableName: 'payments', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
        ],
      }),
      true,
    );
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          { name: 'id', type: 'uuid', isPrimary: true, default: 'uuid_generate_v4()' },
          { name: 'refund_number', type: 'varchar', length: '50', isNullable: false, isUnique: true, default: 'fn_generate_refund_number()' },
          { name: 'order_id', type: 'uuid', isNullable: false },
          ...
          { name: 'amount', type: 'decimal', precision: 14, scale: 2, isNullable: false },
          { name: 'currency_code', type: 'varchar', length: '3', default: `'PKR'` },
          { name: 'refund_method', type: 'refund_method_enum', default: `'original_payment'` },
          { name: 'gateway_refund_id', type: 'varchar', length: '255', isNullable: true },
          { name: 'status', type: 'refund_status_enum', default: `'requested'` },
        ...
        foreignKeys: [
          ...
          new TableForeignKey({ columnNames: ['payment_id'], referencedTableName: 'payments', referencedColumnNames: ['id'] }),
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'card_brand', type: 'varchar', length: '30', isNullable: true },
          { name: 'card_holder_name', type: 'varchar', length: '100', isNullable: true },
          { name: 'expiry_month', type: 'smallint', isNullable: true },
          { name: 'expiry_year', type: 'smallint', isNullable: true },
          { name: 'wallet_number', type: 'varchar', length: '20', isNullable: true },
    ...
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_payments_status ON payments(status, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_payments_transaction ON payments(transaction_id) WHERE transaction_id IS NOT NULL`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_payments_method ON payments(method, status)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_payments_gateway ON payments(gateway_name, status)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_payment_attempts_payment ON payment_attempts(payment_id, attempt_number)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_refunds_order ON refunds(order_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_refunds_user ON refunds(user_id, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_refunds_status ON refunds(status, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_refunds_payment ON refunds(payment_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_refunds_pending ON refunds(created_at DESC) WHERE status = 'requested'`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_saved_payments_user ON saved_payment_methods(user_id) WHERE is_active = TRUE`);

    // Triggers
    await queryRunner.query(`CREATE OR REPLACE TRIGGER trg_payments_updated_at BEFORE UPDATE ON payments FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`);
    await queryRunner.query(`CREATE OR REPLACE TRIGGER trg_refunds_updated_at BEFORE UPDATE ON refunds FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`);
    await queryRunner.query(`CREATE OR REPLACE TRIGGER trg_saved_payments_updated_at BEFORE UPDATE ON saved_payment_methods FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP TRIGGER IF EXISTS trg_saved_payments_updated_at ON saved_payment_methods`);
    await queryRunner.query(`DROP TRIGGER IF EXISTS trg_refunds_updated_at ON refunds`);
