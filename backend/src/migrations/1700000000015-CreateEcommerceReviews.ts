...
export class CreateEcommerceReviews1700000000015 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'moderation_status', type: 'moderation_status_enum', default: `'pending'` },
        ...
        uniques: [new TableUnique({ columnNames: ['user_id', 'order_item_id'] })],
        foreignKeys: [
          ...
          new TableForeignKey({ columnNames: ['moderated_by'], referencedTableName: 'users', referencedColumnNames: ['id'] }),
        ],
      }),
      true,
    );
    ...
    await queryRunner.createTable(
      new Table({
        ...
        foreignKeys: [
          new TableForeignKey({ columnNames: ['review_id'], referencedTableName: 'reviews', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
          new TableForeignKey({ columnNames: ['user_id'], referencedTableName: 'users', referencedColumnNames: ['id'] }),
        ],
      }),
      true,
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'status', type: 'moderation_status_enum', default: `'pending'` },
    ...
    // Indexes
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reviews_product ON reviews(product_id, moderation_status, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reviews_user ON reviews(user_id, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reviews_rating ON reviews(product_id, rating) WHERE moderation_status = 'approved'`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reviews_pending ON reviews(created_at DESC) WHERE moderation_status = 'pending'`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reviews_helpful ON reviews(product_id, helpful_count DESC) WHERE moderation_status = 'approved'`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_reviews_order_item ON reviews(order_item_id)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_review_helpfulness_review ON review_helpfulness(review_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_review_helpfulness_user ON review_helpfulness(user_id)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_review_reports_review ON review_reports(review_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_review_reports_pending ON review_reports(created_at DESC) WHERE status = 'pending'`);

    // Triggers
    await queryRunner.query(`CREATE OR REPLACE TRIGGER trg_reviews_updated_at BEFORE UPDATE ON reviews FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP TRIGGER IF EXISTS trg_reviews_updated_at ON reviews`);
