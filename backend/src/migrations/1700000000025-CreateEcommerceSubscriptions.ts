...
export class CreateEcommerceSubscriptions1700000000025 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      new Table({
        ...
        foreignKeys: [
          ...
          new TableForeignKey({
            ...
            referencedTableName: 'saved_payment_methods',
            referencedColumnNames: ['id'],
          }),
        ],
      }),
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_subscriptions_next_delivery ON subscriptions(next_delivery_date, status) WHERE status = 'active'`,
    );

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_subscription_orders_sub ON subscription_orders(subscription_id, scheduled_date DESC)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_subscription_orders_order ON subscription_orders(order_id) WHERE order_id IS NOT NULL`,
    );

    // Triggers
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_subscriptions_updated_at BEFORE UPDATE ON subscriptions FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
