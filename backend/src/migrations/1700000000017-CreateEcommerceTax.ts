...
export class CreateEcommerceTax1700000000017 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'country_code', type: 'varchar', length: '3', isNullable: false },
          { name: 'state_code', type: 'varchar', length: '10', isNullable: true },
          { name: 'city', type: 'varchar', length: '100', isNullable: true },
          { name: 'postal_code_pattern', type: 'varchar', length: '50', isNullable: true },
          { name: 'is_active', type: 'boolean', default: true },
          { name: 'created_at', type: 'timestamptz', isNullable: false, default: 'NOW()' },
        ],
      }),
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'name', type: 'varchar', length: '100', isNullable: false },
          { name: 'rate', type: 'decimal', precision: 6, scale: 3, isNullable: false },
        ...
        foreignKeys: [
          new TableForeignKey({ columnNames: ['tax_zone_id'], referencedTableName: 'tax_zones', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
          new TableForeignKey({ columnNames: ['tax_class_id'], referencedTableName: 'tax_classes', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
        ],
      }),
      true,
    );
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'tax_name', type: 'varchar', length: '100', isNullable: false },
          { name: 'rate_applied', type: 'decimal', precision: 6, scale: 3, isNullable: false },
          { name: 'taxable_amount', type: 'decimal', precision: 14, scale: 2, isNullable: false },
          { name: 'tax_amount', type: 'decimal', precision: 12, scale: 2, isNullable: false },
          { name: 'created_at', type: 'timestamptz', isNullable: false, default: 'NOW()' },
        ],
        foreignKeys: [
          ...
          new TableForeignKey({ columnNames: ['tax_rate_id'], referencedTableName: 'tax_rates', referencedColumnNames: ['id'] }),
        ],
      }),
      true,
    );
    ...
    // Indexes
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_tax_rates_zone_class ON tax_rates(tax_zone_id, tax_class_id) WHERE is_active = TRUE`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_tax_rates_effective ON tax_rates(effective_from, effective_until)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_order_tax_lines_order ON order_tax_lines(order_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_order_tax_lines_item ON order_tax_lines(order_item_id) WHERE order_item_id IS NOT NULL`);
