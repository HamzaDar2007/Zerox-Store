...
export class CreateEcommerceReturns1700000000013 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      new Table({
        ...
        columns: [
          ...
          },
          {
            ...
            isNullable: false,
            isUnique: true,
            default: 'fn_generate_return_number()',
          },
        ...
        foreignKeys: [
          ...
          new TableForeignKey({
            ...
            referencedTableName: 'return_reasons',
            referencedColumnNames: ['id'],
          }),
          new TableForeignKey({
            columnNames: ['exchange_variant_id'],
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          {
            name: 'status',
            type: 'return_shipment_status_enum',
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_return_requests_seller ON return_requests(seller_id, status, created_at DESC)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_return_requests_status ON return_requests(status, created_at DESC)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_return_requests_pending ON return_requests(created_at DESC) WHERE status = 'requested'`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_return_shipments_request ON return_shipments(return_request_id)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_return_shipments_tracking ON return_shipments(tracking_number) WHERE tracking_number IS NOT NULL`,
    );

    // Triggers
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_return_requests_updated_at BEFORE UPDATE ON return_requests FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_return_shipments_updated_at BEFORE UPDATE ON return_shipments FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS trg_return_shipments_updated_at ON return_shipments`,
    );
    await queryRunner.query(
