...
export class CreateEcommerceAudit1700000000030 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'referrer_url', type: 'varchar', length: '500', isNullable: true },
    ...
    // Indexes
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_audit_logs_user ON audit_logs(user_id, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_audit_logs_entity ON audit_logs(entity_type, entity_id)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_audit_logs_action ON audit_logs(action, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_audit_logs_created ON audit_logs(created_at)`);

    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_activity_logs_user ON user_activity_logs(user_id, created_at DESC) WHERE user_id IS NOT NULL`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_activity_logs_type ON user_activity_logs(activity_type, created_at DESC)`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_activity_logs_entity ON user_activity_logs(entity_type, entity_id) WHERE entity_id IS NOT NULL`);
    await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_activity_logs_created ON user_activity_logs(created_at)`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP FUNCTION IF EXISTS fn_audit_trigger() CASCADE`);
