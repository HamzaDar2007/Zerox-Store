...
export class CreateEcommerceChat1700000000021 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        foreignKeys: [
          ...
          new TableForeignKey({
            columnNames: ['last_message_sender_id'],
            referencedTableName: 'users',
            referencedColumnNames: ['id'],
          }),
        ],
    ...
    await queryRunner.createTable(
      new Table({
        ...
        foreignKeys: [
          ...
          new TableForeignKey({
            columnNames: ['reference_product_id'],
            referencedTableName: 'products',
            referencedColumnNames: ['id'],
          }),
          new TableForeignKey({
            columnNames: ['reference_order_id'],
            referencedTableName: 'orders',
            referencedColumnNames: ['id'],
          }),
        ],
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_conversations_active ON conversations(last_message_at DESC) WHERE status = 'active'`,
    );
    ...
    // Triggers
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_conversations_updated_at BEFORE UPDATE ON conversations FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
