  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        uniques: [
          new TableUnique({ columnNames: ['product_id', 'attribute_id'] }),
        ],
        foreignKeys: [
          new TableForeignKey({
            columnNames: ['product_id'],
          ...
          new TableForeignKey({
            columnNames: ['attribute_option_id'],
            referencedTableName: 'attribute_options',
            referencedColumnNames: ['id'],
            onDelete: 'SET NULL',
          }),
        ],
    ...
    await queryRunner.createTable(
      new Table({
        ...
        uniques: [
          new TableUnique({
            columnNames: ['product_variant_id', 'attribute_id'],
          }),
        ],
        foreignKeys: [
          new TableForeignKey({
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_seller ON products(seller_id, status, created_at DESC) WHERE deleted_at IS NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_category ON products(category_id, status, created_at DESC) WHERE deleted_at IS NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_brand ON products(brand_id, status) WHERE deleted_at IS NULL AND brand_id IS NOT NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_price ON products(price) WHERE status = 'active' AND deleted_at IS NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_rating ON products(avg_rating DESC, total_ratings DESC) WHERE status = 'active' AND deleted_at IS NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_sold ON products(total_sold DESC) WHERE status = 'active' AND deleted_at IS NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_featured ON products(created_at DESC) WHERE is_featured = TRUE AND status = 'active' AND deleted_at IS NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_products_created ON products(created_at DESC) WHERE status = 'active' AND deleted_at IS NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_variants_product ON product_variants(product_id, position) WHERE is_active = TRUE`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_variants_sku ON product_variants(sku) WHERE sku IS NOT NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_variants_options ON product_variants USING GIN (options)`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_product_attrs_attr ON product_attributes(attribute_id, attribute_option_id)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_product_attrs_filter ON product_attributes(attribute_id, value_numeric) WHERE value_numeric IS NOT NULL`,
    );

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_variant_attrs_variant ON variant_attribute_values(product_variant_id)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_variant_attrs_attr ON variant_attribute_values(attribute_id, attribute_option_id)`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_price_history_variant ON price_history(variant_id, created_at DESC) WHERE variant_id IS NOT NULL`,
    );

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_product_questions_product ON product_questions(product_id, created_at DESC) WHERE is_approved = TRUE`,
    );
    await queryRunner.query(
