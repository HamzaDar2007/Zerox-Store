...
export class CreateEcommerceLoyalty1700000000023 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        foreignKeys: [
          ...
          new TableForeignKey({
            columnNames: ['tier_id'],
            referencedTableName: 'loyalty_tiers',
            referencedColumnNames: ['id'],
          }),
        ],
      }),
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_loyalty_tx_type ON loyalty_transactions(type, created_at DESC)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_loyalty_tx_expiry ON loyalty_transactions(expires_at) WHERE expires_at IS NOT NULL AND type = 'earned'`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_loyalty_tx_reference ON loyalty_transactions(reference_type, reference_id)`,
    );
    ...
    // Triggers
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_loyalty_points_updated_at BEFORE UPDATE ON loyalty_points FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
