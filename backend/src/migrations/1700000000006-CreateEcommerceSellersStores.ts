...
export class CreateEcommerceSellersStores1700000000006 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      new Table({
        ...
        foreignKeys: [
          new TableForeignKey({
            ...
            referencedTableName: 'seller_wallets',
            referencedColumnNames: ['id'],
            onDelete: 'CASCADE',
          }),
        ],
    ...
    await queryRunner.createTable(
      new Table({
        ...
        uniques: [
          new TableUnique({ columnNames: ['seller_id', 'snapshot_date'] }),
        ],
        foreignKeys: [
          new TableForeignKey({
            columnNames: ['seller_id'],
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_sellers_active ON sellers(id) WHERE verification_status = 'approved' AND vacation_mode = FALSE`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_stores_active ON stores(id) WHERE is_active = TRUE`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_wallet_tx_type ON wallet_transactions(type, status)`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_seller_docs_status ON seller_documents(status) WHERE status = 'pending'`,
    );

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_seller_violations_seller ON seller_violations(seller_id)`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_seller_violations_open ON seller_violations(severity, created_at DESC) WHERE status = 'open'`,
    );

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_seller_perf_seller_date ON seller_performance_snapshots(seller_id, snapshot_date DESC)`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS trg_wallets_updated_at ON seller_wallets`,
    );
    await queryRunner.query(
