  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        uniques: [
          new TableUnique({ columnNames: ['entity_type', 'entity_id'] }),
        ],
      }),
      true,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_seo_type ON seo_metadata(entity_type)`,
    );

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_redirects_source ON url_redirects(source_url) WHERE is_active = TRUE`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_redirects_target ON url_redirects(target_url)`,
    );

    // Triggers
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_seo_updated_at BEFORE UPDATE ON seo_metadata FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
  }
