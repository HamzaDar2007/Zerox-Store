  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
      ...
      new Table({
        ...
        columns: [
          ...
          { name: 'address_id', type: 'uuid', isNullable: false },
          {
            ...
            isNullable: false,
            isUnique: true,
            default: 'fn_generate_order_number()',
          },
    ...
    await queryRunner.createTable(
      new Table({
        ...
        foreignKeys: [
          ...
          new TableForeignKey({
            columnNames: ['shipped_from_warehouse'],
            referencedTableName: 'warehouses',
            referencedColumnNames: ['id'],
          }),
        ],
    ...
    await queryRunner.createTable(
      new Table({
        ...
        uniques: [
          new TableUnique({ columnNames: ['shipment_id', 'order_item_id'] }),
        ],
        foreignKeys: [
          new TableForeignKey({
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_orders_pending_payment ON orders(created_at) WHERE status = 'pending_payment'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_order_items_seller ON order_items(seller_id, status, created_at DESC)`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_order_status_history_item ON order_status_history(order_item_id, created_at DESC) WHERE order_item_id IS NOT NULL`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_shipments_carrier ON shipments(carrier_code, status)`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_order_snapshots_item ON order_snapshots(order_item_id)`,
    );

    // Triggers
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_orders_updated_at BEFORE UPDATE ON orders FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_order_items_updated_at BEFORE UPDATE ON order_items FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
    await queryRunner.query(
      `CREATE OR REPLACE TRIGGER trg_shipments_updated_at BEFORE UPDATE ON shipments FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp()`,
    );
