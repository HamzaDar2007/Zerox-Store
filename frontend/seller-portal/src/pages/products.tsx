 ...
 * Full CRUD for products with variants and images.
 */
...
import { MoreHorizontal, Pencil, Trash2, Plus, X, ArrowLeft, Layers, Image as ImageIcon, Tag } from 'lucide-react'
...
function VariantsPanel({ productId }: { productId: string }) {
  ...
  const { register, handleSubmit, reset, control, formState: { errors } } = useForm<VariantFormData>({ resolver: zodResolver(variantSchema) as never })
  const createM = useMutation({ mutationFn: (d: VariantFormData) => productsApi.createVariant({ ...d, productId }), onSuccess: () => { qc.invalidateQueries({ queryKey: ['product-variants', productId] }); setDialogOpen(false); reset(); toast.success('Variant created') }, onError: (e) => toast.error(getErrorMessage(e)) })
  const updateM = useMutation({ mutationFn: ({ id, ...d }: VariantFormData & { id: string }) => productsApi.updateVariant(id, d), onSuccess: () => { qc.invalidateQueries({ queryKey: ['product-variants', productId] }); setDialogOpen(false); setEditing(null); toast.success('Updated') }, onError: (e) => toast.error(getErrorMessage(e)) })
  const deleteM = useMutation({ mutationFn: (id: string) => productsApi.deleteVariant(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['product-variants', productId] }); toast.success('Deleted') }, onError: (e) => toast.error(getErrorMessage(e)) })
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? <p className="text-sm text-muted-foreground">Loading…</p> : !(Array.isArray(variants) ? variants : []).length ? <p className="text-sm text-muted-foreground">No variants yet.</p> : (
...
function ImagesPanel({ productId }: { productId: string }) {
  ...
  const uploadM = useMutation({
    mutationFn: (file: File) => { const fd = new FormData(); fd.append('file', file); fd.append('productId', productId); return productsApi.uploadImage(fd) },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['product-images', productId] }); toast.success('Image uploaded') },
    onError: (e) => toast.error(getErrorMessage(e)),
  })
  const deleteM = useMutation({ mutationFn: (id: string) => productsApi.deleteImage(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['product-images', productId] }); toast.success('Deleted') }, onError: (e) => toast.error(getErrorMessage(e)) })

  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold">Images</h3>
      <FileUploader accept="image/*" maxSizeMB={5} onUpload={(files) => { if (files[0]) uploadM.mutate(files[0]) }} />
      {uploadM.isPending && <Progress value={undefined} className="h-1" />}
      {isLoading ? <p className="text-sm text-muted-foreground">Loading…</p> : !(Array.isArray(images) ? images : []).length ? <p className="text-sm text-muted-foreground">No images yet.</p> : (
...
export default function ProductsPage() {
  ...
  const { register, handleSubmit, reset, control, formState: { errors } } = useForm<ProductFormData>({ resolver: zodResolver(productSchema) as never })
  ...
  const updateM = useMutation({
    mutationFn: ({ id, ...d }: ProductFormData & { id: string }) => productsApi.update(id, d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['products'] }); toast.success('Updated') },
    onError: (e) => toast.error(getErrorMessage(e)),
  })
  const deleteM = useMutation({
    mutationFn: (id: string) => productsApi.delete(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['products'] }); setDeleteTarget(null); toast.success('Deleted') },
    onError: (e) => toast.error(getErrorMessage(e)),
  })

  const openCreate = () => {
    setEditingProduct(null); setActiveTab('details')
    reset({ name: '', slug: '', shortDesc: '', fullDesc: '', basePrice: 0, currency: 'USD', categoryId: '', brandId: '', isActive: true, isDigital: false, taxClass: '', status: 'draft' })
    setEditMode('form')
  }
  const openEdit = (p: Product) => {
    setEditingProduct(p); setActiveTab('details')
    reset({ name: p.name, slug: p.slug, shortDesc: p.shortDesc ?? '', fullDesc: p.fullDesc ?? '', basePrice: p.basePrice, currency: p.currency ?? 'USD', categoryId: p.categoryId ?? '', brandId: p.brandId ?? '', isActive: p.isActive, isDigital: p.isDigital ?? false, taxClass: p.taxClass ?? '', status: p.status })
    setEditMode('form')
  }
  const onSubmit = (d: ProductFormData) => {
    const sanitized = { ...d, name: sanitizeText(d.name), shortDesc: d.shortDesc ? sanitizeText(d.shortDesc) : undefined }
    isExisting ? updateM.mutate({ ...sanitized, id: editingProduct.id }) : createM.mutate(sanitized)
  }
  const goBack = () => { setEditMode('list'); setEditingProduct(null) }

  if (profileLoading) return <Loading />
  if (!storeId) return <EmptyState icon={Tag} title="No Store" description="Complete onboarding first." />
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold">{isExisting ? `Edit: ${editingProduct.name}` : 'Create Product'}</h1>
        </div>

        <div className="flex gap-1 border-b">
          {([
            ['details', 'Details', Tag, true],
            ['variants', 'Variants', Layers, isExisting],
            ['images', 'Images', ImageIcon, isExisting],
          ] as const).map(([key, label, Icon, enabled]) => (
            <button key={key} onClick={() => enabled && setActiveTab(key)} disabled={!enabled}
            ...
            <CardContent className="pt-6">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-2"><Label>Base Price</Label><Input type="number" step="0.01" {...register('basePrice')} /></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-4 pt-6">
                    ...
                    <div className="flex items-center gap-2"><Controller name="isDigital" control={control} render={({ field }) => <Switch checked={field.value} onCheckedChange={field.onChange} />} /><Label>Digital</Label></div>
                  </div>
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  <Button type="button" variant="outline" onClick={goBack}>Cancel</Button>
                  <Button type="submit" disabled={createM.isPending || updateM.isPending}>{isExisting ? 'Update' : 'Create'}</Button>
                </div>
                {!isExisting && <p className="text-xs text-muted-foreground text-center">Save first to unlock Variants & Images tabs</p>}
              </form>
            </CardContent>
          </Card>
        )}

        {activeTab === 'variants' && isExisting && <VariantsPanel productId={editingProduct.id} />}
        {activeTab === 'images' && isExisting && <ImagesPanel productId={editingProduct.id} />}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Products" description="Manage your product catalog" action={{ label: 'Add Product', onClick: openCreate }} />
