...
export default function CategoriesPage() {
  ...
  const deleteM = useMutation({ mutationFn: (id: string) => categoriesApi.delete(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['categories'] }); setDeleteTarget(null); toast.success('Deleted') }, onError: (e) => toast.error(getErrorMessage(e, 'Failed')) })
  ...
  const uploadImageM = useMutation({
    ...
    onError: (e) => toast.error(getErrorMessage(e, 'Image upload failed')),
  })

  const openCreate = () => { setEditing(null); reset({ name: '', slug: '', description: '', imageUrl: '', sortOrder: 0, isActive: true }); setDialogOpen(true) }
  const openEdit = (c: Category) => { setEditing(c); reset({ name: c.name, slug: c.slug, description: c.description ?? '', imageUrl: c.imageUrl ?? '', sortOrder: c.sortOrder, isActive: c.isActive }); setDialogOpen(true) }
  ...
  const columns: ColumnDef<Category>[] = [
    { accessorKey: 'name', header: ({ column }) => <SortHeader column={column}>Name</SortHeader>, cell: ({ row }) => (
      <div className="flex items-center gap-2">
        {row.original.imageUrl && <img src={row.original.imageUrl} alt={row.original.name} className="h-8 w-8 rounded object-cover" />}
      ...
      id: 'actions', cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={() => setUploadTarget(row.original)}><ImageIcon className="mr-2 h-4 w-4" />Upload Image</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        enableRowSelection
        onBulkDelete={(rows) => {
          Promise.allSettled(rows.map((r) => categoriesApi.delete(r.id))).then((results) => {
            qc.invalidateQueries({ queryKey: ['categories'] })
            const failed = results.filter((r) => r.status === 'rejected').length
            if (failed) toast.error(`${failed} of ${rows.length} failed to delete`)
            else toast.success(`${rows.length} category(ies) deleted`)
          })
        }}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2"><Label>Image</Label><Controller name="imageUrl" control={control} render={({ field }) => <UrlFileField value={field.value ?? ''} onChange={field.onChange} />} /></div>
      ...
      {uploadTarget && (
        <Dialog open={!!uploadTarget} onOpenChange={() => setUploadTarget(null)}>
          <DialogContent>
            <DialogHeader><DialogTitle>Upload Category Image</DialogTitle><DialogDescription>{uploadTarget.name}</DialogDescription></DialogHeader>
            <div className="space-y-4">
              {uploadTarget.imageUrl && <img src={uploadTarget.imageUrl} alt={uploadTarget.name} className="h-32 w-full rounded-md object-cover" />}
              <FileUploader accept="image/*" maxSizeMB={5} preview={uploadTarget.imageUrl} onUpload={(files) => { if (files[0]) uploadImageM.mutate({ id: uploadTarget.id, file: files[0] }) }} />
